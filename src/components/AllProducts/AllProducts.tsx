...
const AllProducts: React.FC = () => {
    ...
    return (<>
        <Flex m={2} mt={5} mb={5} className='AllProductstotal' direction="column" height={"auto"} borderRadius={8}>
            <Flex alignItems={"center"} justifyContent="space-between" width={"auto"} height={"7vh"} m={1} p={2}>
                <Flex fontSize={"2xl"} fontWeight="bold" ml={4}>
                    All Products
                </Flex>
                <Flex pr={5} pt={"2rem"}>
                    <a className='viewAlltotal' href='/all'><Flex fontSize={"1.1rem"}>View All</Flex></a>
                </Flex>
            </Flex>
            <Flex height={"auto"} alignItems="center">
                {getAllProductsStatus === "FETCHING" ||  getAllProductsStatus === "NOT_STARTED"? <Flex alignItems={"center"} justifyContent={"center"} width={"100vw"} height={"46.7vh"}><Spinner size={"xl"} /></Flex> : <>
                    <Flex width={"0%"} height={"100%"} display={pageNumber > 1 ? "" : "none"} fontSize="4xl">
                        <MdArrowBackIosNew className='pos_lefttotal' onClick={() => setPageNumber(pageNumber - 1)} />
                    </Flex>
                    ...
                    <Flex width={"0%"} height={"100"} display={pageNumber >= (dataAllProducts.total.length / itemsPerPage) ? "none" : ""} fontSize={"4xl"}>
                        <MdArrowForwardIos className="pos_righttotal" onClick={() => setPageNumber(pageNumber + 1)} />
                    </Flex></>}
            </Flex>
        </Flex>
    </>)
